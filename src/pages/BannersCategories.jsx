...
import {
  getBannerCategoryAPI,
  addBannerCategoryAPI,
  delBannerCategoryByIdAPI,
  loadOneByIdAPI,
  modifyBannerCategoryByIdAPI,
} from "../services/bannerCategory";
import { dalImg } from "../utils/tools";
import MyUpload from "../components/MyUpload";

function BannersCategories() {
  ...
  const loadData = async () => {
    const res = await getBannerCategoryAPI(page);
    setList(res.data);
    setTotal(res.total);
  };
  const columns = [
        ...
        return (
          <Space>
            ...
            <Popconfirm
              ...
              onConfirm={async () => {
                console.log(c.id);
                await delBannerCategoryByIdAPI(c.id);
                setQuery({});
    ...
    >
      ...
      >
        <Form
          ...
          onFinish={async (v) => {
            if (currentId > -1) {
              await modifyBannerCategoryByIdAPI(currentId, {
                ...v,
                coverImage: imageUrl,
              });
            } else {
              await addBannerCategoryAPI({ ...v, coverImage: imageUrl });
            }
